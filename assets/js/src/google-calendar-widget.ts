...
interface GoogleCalendarWidgetLocalization {
  all_day?: string;
 ...
 */
const google_calendar_widget = (($: JQueryStatic) => {
  'use strict';
  ...
  function formatEventDetails(titleFormat: string, event: GoogleCalendarEvent): string {
    ...
    function replaceSTARTTIME(strMatchingString: string, strGroup1: string, strGroup2: string): string {
      return startTimeString ? strGroup1 + startTimeString + strGroup2 : "";
    }

    function replaceENDTIME(strMatchingString: string, strGroup1: string, strGroup2: string): string {
   ...
   */
  function createClickHandler(item: HTMLElement, entry: GoogleCalendarEvent): (e: { preventDefault: () => void }) => void {
  ...
  function createListEvents(
      ...
      while (output.length < maxResults) {
        ...
        if (firstStartTime !== null && firstStartIndex !== null) {
          ...
          if (output.length > 0) {
            ...
            if (lastStartDate && (lastStartDate.getTime() === firstStartTime.getTime()) && (lastUid === uid)) {
              // This is a duplicate.
              log("Duplicate event");
              uniqueEntry = false;
            }
    ...
    batch.then(function(resp: BatchResponse) {
 ...
 */
function google_calendar_widget_google_init(): void {
  // The Google Client API is ready to be used.
